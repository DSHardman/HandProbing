...
# Randomly select a point to probe, and plot location
def choose_random_point(coord):
    polygon = Polygon(coords)

    while 1:  # Loop until the random location falls within the hand polygon
...
def pressrecord(x, y, savestring):
    ...
    # Wait for OpenEIT board to collect unpressed data
    ...
    # Collect and save pressed data from OpenEIT board
    serial_state = serial_handler.updater
    while serial_handler.updater == serial_state:
        pass  # wait for last set of readings to end
...
# Main loop: continuously randomise, pinch, plot, and save data, changing sides where necessary
coord = [0, 0]
